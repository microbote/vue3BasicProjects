<template>
  <div class="abody">
    ...
    <div class="cup">
      <div class="remained" id="remained" ref="remained">
        <span id="liters" ref="liters"></span>
        <small>Remained</small>
      </div>

      <div class="percentage" id="percentage" ref="percentage"></div>
    </div>
...
onMounted(() => {
  smallCups.value = document.querySelectorAll('.cup-small')
  updateBigCup()
  smallCups.value.forEach((cup, idx) => {
    cup.addEventListener('click', () => highlightCups(idx))
  })

  function highlightCups(idx) {
    if (idx === 7 && smallCups.value[idx].classList.contains('full')) idx--
    else if (
      smallCups.value[idx].classList.contains('full') &&
      !smallCups.value[idx].nextElementSibling.classList.contains('full')
    ) {
      idx--
    }
  ...
  function updateBigCup() {
    ...
    if (fullCups === totalCups) {
      ...
      liters.value.innerText = `${2 - (250 * fullCups) / 1000}L`
    }
  }
})
</script>
